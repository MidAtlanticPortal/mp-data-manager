show_layertype_form = function(layertype) {
  ...
  if (url.length > 0) {
    ...
    if ($('#id_layer_type').val() == "WMS" && $('#id_wms_help').is(':checked')) {
...
get_wms_capabilities = function() {
  if ($('#id_layer_type').val() == "WMS" && $('#id_url').length > 0 && $('#id_wms_help').prop('checked') == true) {
    show_spinner();
    var url = $('#id_url').val();
    $.ajax({
        ...
        success: function(data) {
          ...
          if (style_keys.length == 0) {
            ...
            var style_val = $('#id_wms_styles').val();
            var style_html = '<select id="id_wms_styles" name="wms_srs">';
